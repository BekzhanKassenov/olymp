...
// Class for unsigned long arythmetic
class BigInt {
public:
...
private:
    ...
    void __sub_this(const BigInt& _rhs);
...
BigInt::BigInt(const std::vector <BigInt::uint32>& _digits) {
    __init_with_vector(_digits);
}

BigInt::BigInt(BigInt::uint64 _number) {
...
/* Initialization helpers */
void BigInt::__init_with_number(BigInt::uint64 _number) {
    _digits.clear();
...
void BigInt::__sum_this(const BigInt& _rhs) {
    ...
    for (std::size_t i = _digits.size(); i < _rhs._digits.size(); i++) {
        _carry += _rhs._digits[i];
...
void BigInt::__sub_this(const BigInt& _rhs) {
    // _rhs._digits.size() <= this->_digits.size()
    ...
    for (std::size_t i = 0; i < _rhs._digits.size(); i++) {
        ...
        if (_digits[i] < _rhs._digits[i]) {
            _digits[i] += BigInt::BASE;
 ...
 * Works with assumption that _number < BASE
 */
...
/*
 * Does not work when BigInt::BASE * _number > LONG_LONG_MAX
 */
void BigInt::__mod_this(BigInt::uint64 _number) {
    BigInt::uint64 _carry = 0;

    for (auto it = _digits.crbegin(); it != _digits.crend(); it++) {
        _carry *= BigInt::BASE;
        _carry += *it;
        _carry %= _number;
    }
 ...
 */
BigInt power(BigInt _base, BigInt::uint64 _power) {
...
bool BigInt::less(const BigInt& _lhs, const BigInt& _rhs) {
    // References to arrays of digits
 ...
 * ================================================= *
 */
inline std::ostream& operator << (std::ostream& _output_stream, const BigInt& _number) {
    _output_stream << _number.to_string();

    return _output_stream;
}

inline std::istream& operator >> (std::istream& _input_stream, BigInt& _number) {
