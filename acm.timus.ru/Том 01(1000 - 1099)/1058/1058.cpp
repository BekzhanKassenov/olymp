	...
	for (int i = pos + 1; cur < n; i++)
		{
			...
			cur++;
			if (comp(tmpArea, area - tmpArea) != -1)
				break;
		}

	double ar1 = area - tmpArea;

	tmpArea -= fabs((mn[cur % n] - a) * (mn[(cur - 1) % n] - a));

	double ar2 = area - tmpArea - ar1;

	if (comp(ar2, 0) != 0)
		{
			double x = (fabs((area / 2 - tmpArea) / (ar2))) / 2;
			...
			get_line(a, b, c, mn[pos], mn[(pos + 1) % n]);

			point vect = point(b, -a);

			vect = make_len(vect, x);

			point qwe = mn[pos] + vect;
